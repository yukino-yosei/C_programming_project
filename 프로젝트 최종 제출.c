...
int main(){
    ...
    for(int i = 1;i <= 6;++i){
        BACK:
        printf("%d반의 학생 수를 입력해주렴. 부탁하마.\n====>   ",i);
        while (scanf("%d", student + i) != 1) {
            int c;
            while ((c = getchar()) != '\n' && c != EOF);
            printf("숫자로 다시 입력 부탁하마. 마리네뜨.\n");
        }
        if(student[i] >= 40){
            printf("뭔 학생수가 그리 많니? 다시 입력하렴.\n");
            goto BACK;//10
        }
        if(student[i] <= 0){
            printf("학생수가 그렇게 되는게 말이 돼? 다시 입력하렴.\n");
            goto BACK;//9
        }
        for(int j = 1;j <= student[i];++j){
            printf("%d반 %d번 학생의 성적을 입력해주렴. 부탁하마.(국어-영어-수학-한국사-사회탐구-과학탐구 순서)\n====>   ",i,j);
            for(int k = 0;k < 6;++k){
        ...
        switch (mode){
            case 1:
                ...
                if((a <= 0 || a > 6) || b > student[a]){
                    printf("반, 번호가 뭔가 잘못됐구나, 마리네뜨. 다시 부탁한다.\n");
                    goto BACK1;//8
                }
            ...
            case 4:
                ...
                if(a <= 0 || a > 6){
                    printf("반이 뭔가 잘못됐구나, 마리네뜨. 다시 부탁한다.\n");
                    goto BACK4;//3
                }
                printf("조회하고 싶은 과목을 입력해주려무나.\n1. 국어\n2. 영어\n3. 수학\n4. 한국사\n5. 사회탐구\n6. 과학탐구\n7. 전체성적\n===>   ");
                while (scanf("%d",&s) != 1) {
